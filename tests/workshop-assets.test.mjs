...
test('the landing page applies the supplied brand while preserving its paper journey', async () => {
  ...
  assert.match(entry, /Persona Lab/);
  assert.match(entry, /Meet your next product decision\./);
  assert.match(entry, /workshop\/logos\/persona-lab\.svg/);
